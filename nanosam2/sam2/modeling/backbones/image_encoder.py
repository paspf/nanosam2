...
if __name__ == "__main__":
    from sam2.modeling.backbones.casvit import RCViT
    from sam2.modeling.position_encoding import PositionEmbeddingSine
    trunk = RCViT(layers=[3, 3, 6, 3], embed_dims=[96, 128, 256, 512], mlp_ratios=4, 
        downsamples=[True, True, True, True],
        norm_layer=nn.BatchNorm2d, attn_bias=False, act_layer=nn.GELU, drop_rate=0., fork_feat=True)
    
    neck = FpnNeck(
        position_encoding=PositionEmbeddingSine(num_pos_feats=64, normalize=True),
        d_model=256,
        backbone_channel_list=[512, 256, 128, 96],
        fpn_interp_model="nearest",
        fuse_type="sum",
        fpn_top_down_levels=[2,3],
    )
    image_encoder = ImageEncoder(trunk, neck, scalp=1).cuda()
    test_input = torch.rand(1, 3, 1024, 1024).cuda()
    # image_encoder = torch.compile(image_encoder)
    output = image_encoder(test_input)
